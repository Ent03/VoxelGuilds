...
public class TeamPermissionRemove extends CustomCommand {
    public TeamPermissionRemove() {
        super("remove", "/c permission remove <member> <permission>", "Remove a permission from a member");
    ...
    protected boolean onCommand(CommandSender commandSender, String s, String[] args) throws Exception {
        ...
        if(!member.hasPermission(permission)){
            ServiceManager.getService(TeamService.class).messagePlayer(player, "§cMember doesn't have this permission!");
            return true;
        }
        else {
            member.removeExplicitPermission(permission);
            ServiceManager.getService(TeamService.class).messagePlayer(player, "§7Permission §c" + permission.getKey() + "§7 removed from §a" + target.getName());
        }
        return true;
    }
    ...
    public List<String> tabComplete(CommandSender sender, String alias, String[] args) throws IllegalArgumentException {
        ...
        if(args.length == 1){
            ...
            OfflinePlayer target = Bukkit.getOfflinePlayerIfCached(args[0]);
            if(target == null) return super.tabComplete(sender,alias,args);
            return StringUtil.copyPartialMatches(args[1], team.getMember(target.getUniqueId()).getAllPermissionKeys(), new ArrayList<>());
        }
